...
import { TRUST_ALGORITHM_VERSION, TRUST_EVENT_SCHEMA_VERSION, TRUST_SCHEMA_VERSION, } from './index.js';
import { PROMISE_OUTCOME_ATTESTATION_VERSION, PROMISE_OUTCOME_EAS_SCHEMA, } from './promise-outcome-types.js';
...
export function getPromiseOutcomeSchemaUid(resolver, revocable) {
    const digest = hashBytes32(solidityPacked(['string', 'address', 'bool'], [PROMISE_OUTCOME_EAS_SCHEMA, resolver, revocable]));
    if (digest === null)
        throw new TypeError('Unable to derive EAS schema UID');
    return digest;
}
export function createPromiseOutcomeAttestation(input) {
    ...
    const eventIdentity = hashBytes32(CODER.encode(['string', 'uint64'], [event.gameId, event.sequence]));
    if (eventDigest === null || eventIdentity === null || !isHexData(encodedPublic))
        return createRejected();
    const encodedData = CODER.encode([...EVENT_TYPES, 'bytes32'], [...eventValues(event), eventDigest]);
    if (!isHexData(encodedData))
        return createRejected();
    const attestation = {
...
export function decodePromiseOutcomeEasData(data) {
    if (!isHexData(data))
    ...
    const event = parseEvent({
        ...
        observedAt: typeof decoded[7] === 'bigint' && decoded[7] <= BigInt(Number.MAX_SAFE_INTEGER)
...
//# sourceMappingURL=promise-outcome.js.map