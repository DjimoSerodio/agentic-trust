...
import { parseWalletBindingRecord, parseWalletBindingSigningRequest, } from './wallet-binding-parse.js';
export function createMemoryNonceConsumer() {
...
export async function createWalletBindingRecord(request, signer) {
...
export function verifyWalletBindingRecord(input, expected, nonceConsumer) {
    const parsed = parseWalletBindingRecord(input);
    if (parsed.kind === 'rejected')
        return parsed;
    const { record } = parsed;
    if (record.did !== expected.did)
        return { kind: 'rejected', reason: 'expected-did' };
    if (record.domain !== expected.domain)
        return { kind: 'rejected', reason: 'expected-domain' };
    if (record.uri !== expected.uri)
        return { kind: 'rejected', reason: 'expected-uri' };
    if (record.chainId !== expected.chainId)
        return { kind: 'rejected', reason: 'expected-chain-id' };
    if (record.nonce !== expected.nonce)
        return { kind: 'rejected', reason: 'expected-nonce' };
    ...
    if (ageMs > expected.maxAgeMs)
        return { kind: 'rejected', reason: 'stale' };
    if (ageMs < -expected.maxFutureSkewMs)
        return { kind: 'rejected', reason: 'future' };
