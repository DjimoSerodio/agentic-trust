...
function getAttestationUid(receipt, request, config) {
    if (receipt === null)
        return null;
...
function normalizeEthersAttestation(value) {
...
export function createEthersEasGateway(config, contract) {
    return {
        config,
...
function createUid(request, config, time) {
    const hash = keccak256(solidityPacked(['bytes32', 'address', 'address', 'uint64', 'uint64', 'bool', 'bytes32', 'bytes', 'uint32'], [
        request.schema,
        request.recipient,
        config.attester,
        time,
...
export function createInMemoryEasGateway(config) {
    ...
    return {
        ...
        tamper(uid, changes) {
            ...
            if (record === undefined)
                return;
            if (isBytes32(changes.uid))
                record.uid = changes.uid;
            if (isBytes32(changes.schema))
                record.schema = changes.schema;
            if (typeof changes.time === 'bigint')
                record.time = changes.time;
            ...
            if (isBytes32(changes.refUid))
                record.refUid = changes.refUid;
            if (isAddressValue(changes.recipient))
                record.recipient = changes.recipient;
            if (isAddressValue(changes.attester))
                record.attester = changes.attester;
            if (typeof changes.revocable === 'boolean')
                record.revocable = changes.revocable;
            if (isHexData(changes.data))
                record.data = changes.data;
        },
    };
}
//# sourceMappingURL=eas-gateway.js.map